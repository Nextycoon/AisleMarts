...
export default function Orders(){
  ...
  return (
    <ScrollView style={{flex:1, padding:16}}>
      ...
      {orders.map(o=>(
        <TouchableOpacity key={o.id} style={{backgroundColor:'#121A2A', padding:14, borderRadius:12, marginTop:10}}>
          <View style={{flexDirection:'row', justifyContent:'space-between'}}>
            <Text style={{color:'#fff', fontWeight:'700'}}>{o.id}</Text>
            {badge(o.status)}
          </View>
          <Text style={{color:'#B8C2CC'}}>{o.customer} • {o.createdAt} • ${o.total.toFixed(2)}</Text>
        </TouchableOpacity>
      ))}
    </ScrollView>
  );
