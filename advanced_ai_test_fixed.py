...
class AdvancedAITester:
    ...
    async def test_ai_content_generation(self):
        ...
        # Fix the brand_voice structure - values should be a string, not a list
        brand_voice = {
            "personality": "innovative",
            "values": "sustainability, quality, inclusivity",  # Changed from list to string
            "tone": "approachable"
        }
    ...
    async def run_all_tests(self):
        ...
        print("-" * 50)
        capabilities_tested = [
            ("Advanced AI Engine Health Check", "health"),
            ("Visual Product Recognition with Style Analysis", "visual"), 
            ("User Behavior Analysis with Predictive Insights", "behavior"),
            ("Emotional Intelligence & Mood-Based Commerce", "emotional"),
            ("Trend Prediction with Market Intelligence", "trend"),
            ("Content Virality Prediction & Optimization", "virality"),
            ("Personalized Recommendations Engine", "recommendations"),
            ("Smart Pricing Optimization with Dynamic Triggers", "pricing"),
            ("AI Content Generation for Social Commerce", "content"),
            ("AI Insights Engine with Business Intelligence", "insights")
        ]
        
        for capability, keyword in capabilities_tested:
            status = "✅" if any(keyword.lower() in result["test"].lower() and result["success"] for result in self.test_results) else "❌"
